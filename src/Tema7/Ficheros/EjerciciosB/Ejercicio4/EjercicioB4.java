...
public class EjercicioB4 {
    static void main() throws IOException {
        ...
        List<String> listaNombres = Files.readAllLines(Paths.get("usa_nombres.txt"));
        List<String> listaApellidos = Files.readAllLines(Paths.get("usa_apellidos.txt"));
        for (int i = 0; i < numNom; i++) {
            String nombreRandom = listaNombres.get(rand.nextInt(listaNombres.size()));
            String apellidoRandom = listaNombres.get(rand.nextInt(listaNombres.size()));
